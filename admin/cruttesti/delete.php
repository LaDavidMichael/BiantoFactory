...
if(isset($_GET['id'])){
	...
	if(mysqli_num_rows($cek) > 0){
		...
		if($del){
			echo '<script>alert("Berhasil menghapus data."); document.location="../dashboard.php?page=tampiltesti";</script>';
		}else{
			echo '<script>alert("Gagal menghapus data."); document.location="dashboard.php?page=tampiltestik";</script>';
		}
	}else{
		echo '<script>alert("ID tidak ditemukan di database."); document.location="dashboard.php?page=tampiltesti";</script>';
	}
}else{
	echo '<script>alert("ID tidak ditemukan di database."); document.location="dashboard.php?page=tampiltesti";</script>';
}
